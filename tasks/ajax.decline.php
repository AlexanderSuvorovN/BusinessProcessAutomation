<?php
	...
	try
	{
		...
		$s = $st->dbh->prepare("UPDATE `tasks` SET `status` = 'declined' WHERE `id` = :task_id");
		$s->bindParam(':task_id', $task_id);
		$s->execute();
		// создадим оповещение автору задачи
		// вначале сформируем текст сообщения
		$message = 'Пользователь <a href=\'/users/view?id='.$st->user['id'].'\'>'.$st->user['person_firstname'].' '.$st->user['person_lastname'].'</a> отклонил задачу <a href=\'/tasks/view?id='.$task_id.'\'>'.$task_id.'</a>';
	...
	catch(Exception $e)
	{
		...
		$st->db_close();
		$json['status'] = 'failed';
		$json['message'] = 'task decline operation failed:'.$e->getMessage();
		echo json_encode($json);
		exit();
	}