<?php 
	...
	try
	{
		...
		$s = $st->dbh->prepare("SELECT `u`.`id` AS `user_id`, `p`.`firstname` AS `person_firstname`, `p`.`lastname` AS `person_lastname`, `terminal_username` AS `user_terminal_username` FROM `users` AS `u` JOIN `persons` AS `p` ON (`u`.`person_id` = `p`.`id`) JOIN `employees` AS `e` ON (`p`.`id` = `e`.`person_id`)");
		$s->execute();
